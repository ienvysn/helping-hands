...
const getAllOpportunities = async (req, res) => {
  try {
    ...
    if (minHours || maxHours) {
      filter.durationHours = {};
      if (minHours) filter.durationHours.$gte = parseInt(minHours);
      if (maxHours) filter.durationHours.$lte = parseInt(maxHours);
    }
...
const updateOpportunity = async (req, res) => {
  try {
    ...
    if (req.file) {
      opportunity.imageUrl = `/images/${req.file.filename}`;
    }

    // Update fields and parse strings to numbers
    if (title !== undefined) opportunity.title = title;
    if (description !== undefined) opportunity.description = description;
    if (tasks !== undefined) opportunity.tasks = tasks;
    if (requirements !== undefined) opportunity.requirements = requirements;
    if (eventDate !== undefined) opportunity.eventDate = new Date(eventDate);
    if (startTime !== undefined) opportunity.startTime = startTime;
    if (durationHours !== undefined) opportunity.durationHours = Number(durationHours);
    if (opportunityType !== undefined) opportunity.opportunityType = opportunityType;
    if (cause !== undefined) opportunity.cause = cause;
    if (location !== undefined) opportunity.location = location;
    if (maxVolunteers !== undefined) opportunity.maxVolunteers = maxVolunteers ? Number(maxVolunteers) : null;
    if (isActive !== undefined) opportunity.isActive = isActive;
...
const confirmAllSignups = async (req, res) => {
  try {
    ...
    const pendingSignups = await Signup.find({ opportunityId, status: "pending" }).populate("volunteerId");
    ...
    res.json({ success: true, message: "All signups confirmed" });
  } catch (error) {
    console.error("Confirm all signups error:", error);
    res.status(500).json({ success: false, message: "Error confirming signups", error: error.message });
  }
};
...
const confirmOneSignup = async (req, res) => {
  try {
    ...
    const signup = await Signup.findOne({ opportunityId, volunteerId, status: "pending" }).populate("volunteerId");
    if (!signup) return res.status(404).json({ success: false, message: "Signup not found" });
    ...
    res.json({ success: true, message: "Signup confirmed", data: signup });
  } catch (error) {
    console.error("Confirm signup error:", error);
    res.status(500).json({ success: false, message: "Error confirming signup", error: error.message });
  }
};
  ...
  try {
    ...
    if (signup.volunteerId?.userId) {
      await createNotification(
        ...
        "signup_rejected",
        "Signup Rejected",
        `Your signup for "${opportunity.title}" was not accepted.`
      );
    }
    ...
    res.status(500).json({ success: false, message: "Error rejecting signup", error: error.message });
  }
};
...
const markAttendance = async (req, res) => {
  try {
    ...
    for (const item of attendance) {
      const signup = await Signup.findById(item.signupId).populate("volunteerId");
      if (!signup || signup.status !== "confirmed") continue;

      if (item.attended) {
        const oldHours = signup.volunteerId.totalHours;
        const oldLevel = calculateLevel(oldHours);
        ...
        const newLevel = calculateLevel(signup.volunteerId.totalHours);

        if (signup.volunteerId.userId) {
          await createNotification(signup.volunteerId.userId, "hours_confirmed", "Hours Confirmed", `Awarded ${signup.hoursAwarded} hours!`);
          if (newLevel > oldLevel) {
            await createNotification(signup.volunteerId.userId, "level_up", "Level Up!", `Reached Level ${newLevel}!`);
          }
        }
        confirmedCount++;
      } else {
    ...
    res.json({ success: true, data: { confirmed: confirmedCount, noShows: noShowCount } });
  } catch (error) {
    console.error("Mark attendance error:", error);
