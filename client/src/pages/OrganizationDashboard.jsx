...
const OrganizationDashboard = () => {
  ...
  // UPDATED HANDLE SUBMIT TO USE FORMDATA FOR IMAGE UPLOAD
  const handleSubmit = async () => {
    // Validation
    if (!formData.title || !formData.description || !formData.eventDate) {
      alert("Please fill in all required fields (marked with *)");
    ...
    try {
      ...
      if (isEditing && editingId) {
        // Update opportunity
        res = await fetch(`http://localhost:5000/api/opportunities/${editingId}`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
            // Boundary is set automatically by the browser for FormData
          },
          body: dataToSend,
        });
      } else {
        // Create opportunity
        res = await fetch("http://localhost:5000/api/opportunities", {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (res.ok && data.success) {
        if (isEditing) {
          ...
          // Add new one to list (assuming the back-end returns the created object)
